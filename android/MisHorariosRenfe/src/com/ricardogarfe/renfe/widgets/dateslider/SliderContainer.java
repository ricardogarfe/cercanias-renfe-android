...
public class SliderContainer extends LinearLayout {
     ...
     * 
     * @param c
     *            the minimum date (inclusiv)
     */
    public void setMinTime(Calendar c) {
        if (mTime == null) {
            throw new RuntimeException(
                    "You have to call setTime before setting a MinimumTime!");
        }
        final int childCount = getChildCount();
        for (int i = 0; i < childCount; i++) {
            View v = getChildAt(i);
     ...
     * 
     * @param source
     *            The ScrollLayout that generated the time change, or null if
    ...
    private void arrangeScrollers(ScrollLayout source) {
        ...
        if (mOnTimeChangeListener != null) {
            if (minuteInterval > 1) {
                int minute = mTime.get(Calendar.MINUTE) / minuteInterval
                        * minuteInterval;
