 ...
 * types of methods -- add*() methods that add a number of units to a time and
 * return the result as a Calendar, and get*() objects that take a Calendar
 * object and a format string and produce the appropriate TimeObject.
 */
class Util {
    ...
    public static TimeObject getYear(Calendar c, String formatString) {
        ...
        return new TimeObject(String.format(formatString, c, c), startTime,
                endTime);
    }
    ...
    public static TimeObject getMonth(Calendar c, String formatString) {
        ...
        c.set(year, month, c.getActualMaximum(Calendar.DAY_OF_MONTH), 23, 59,
                59);
        c.set(Calendar.MILLISECOND, 999);
        long endTime = c.getTimeInMillis();
        return new TimeObject(String.format(formatString, c, c), startTime,
    ...
    public static TimeObject getMinute(Calendar c, String formatString,
        ...
        c.set(year, month, day, hour, Math.min(59, minute + minInterval - 1),
                59);
        c.set(Calendar.MILLISECOND, 999);
        long endTime = c.getTimeInMillis();
    ...
    public static TimeObject getTime(Calendar c, String formatString,
            int minuteInterval) {
        int year = c.get(Calendar.YEAR);
        int month = c.get(Calendar.MONTH);
        int day = c.get(Calendar.DAY_OF_MONTH);
